...
const FROM_EMAIL = process.env.FROM_EMAIL || 'Script Go <noreply@50scripts.com>';
...
function baseTemplate(content: string): string {
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Script Go</title></head>
...
<p style="margin:0;font-size:12px;color:#6B7280;">Você está recebendo este email porque é usuário do Script Go.</p>
</td></tr>
...
export async function sendWelcomeEmail(to: string, name: string): Promise<EmailResult> {
  const html = baseTemplate(`
    ${heading(`Bem-vindo ao Script Go, ${name}!`)}
  ...
  `);
  return sendEmail(to, 'Bem-vindo ao Script Go!', html);
}
...
export async function sendPasswordResetEmail(to: string, resetLink: string): Promise<EmailResult> {
  ...
  `);
  return sendEmail(to, 'Redefinir sua senha - Script Go', html);
}
