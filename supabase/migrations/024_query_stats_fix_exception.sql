-- Corrige 00139_query_stats.sql: a query contra pg_stat_statements não
-- estava protegida por exception handler (só a tentativa de CREATE EXTENSION
-- estava). Se a extensão aparece em pg_extension mas a consulta falha em
-- runtime (ex: sem privilégio pg_read_all_stats), a função inteira lançava
-- exceção e get_advisor_summary() virava null. Agora captura o erro e
-- devolve available:false + error com o motivo, em vez de derrubar tudo.
