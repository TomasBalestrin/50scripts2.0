-- Corrige 00140: get_advisor_summary() roda com `set search_path = public`,
-- mas no Supabase extensões (inclusive pg_stat_statements) costumam ser
-- instaladas no schema `extensions`, não em `public` — por isso a query
-- falhava com "relation pg_stat_statements does not exist" mesmo com a
-- extensão de fato instalada. Agora descobre o schema real via pg_extension
-- + pg_namespace e monta a query dinamicamente com esse schema qualificado,
-- em vez de assumir `public` ou `extensions` fixo.
...
begin
  ...
  if v_pgss_schema is not null then
      ...
      execute format(
        $f$
          select jsonb_build_object(
            'available', true,
            'top_queries', coalesce(jsonb_agg(t), '[]'::jsonb)
          )
          from (
            select
    ...
    exception when others then
      v_query_stats := jsonb_build_object('available', false, 'top_queries', '[]'::jsonb, 'error', sqlerrm);
    end;
  else
    v_query_stats := jsonb_build_object('available', false, 'top_queries', '[]'::jsonb);
  end if;
