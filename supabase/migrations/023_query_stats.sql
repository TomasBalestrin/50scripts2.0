-- Tenta habilitar pg_stat_statements (idempotente). Pode falhar se a
-- extensão não estiver em shared_preload_libraries do servidor — nesse caso
-- get_advisor_summary() reporta query_stats.available = false em vez de
-- quebrar.
do $$
begin
...
-- Estende get_advisor_summary() com as queries mais custosas (via
-- pg_stat_statements) — dá visibilidade de QUAL query pesa, não só "quantos
-- seq scans" (que é agregado por tabela, sem contexto de query). Aditivo —
-- mesma assinatura, só ganha 1 chave nova no jsonb de retorno.
