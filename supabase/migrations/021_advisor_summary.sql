-- Status SDK (Hub Bethel Sistemas, nível 2+): resumo de "advisors" de performance
-- (equivalente aos lints da Supabase) + seq_scan agregado, consumido pela rota
-- GET /api/status-check. Lógica portada fielmente de supabase/splinter (splinter.sql,
-- fonte pública oficial da Supabase) — não é uma aproximação própria.
--
-- Aditivo: só cria a função, nenhuma tabela nova. SECURITY DEFINER pra poder ler
-- pg_catalog.pg_policy/pg_stat_user_tables mesmo chamada via service_role.
...
declare
  ...
  v_rows_total bigint;
begin
  -- 1. auth_rls_initplan: policies que reavaliam auth.<fn>()/current_setting() por linha.
  with policies as (
    select
  ...
  -- 2. multiple_permissive_policies: >1 policy permissiva pra mesma tabela/role/ação.
  with grouped as (
  ...
  -- 3. unindexed_foreign_keys: FK sem índice cobrindo as colunas.
  ...
  -- 4. seq_scan agregado (schema public — tabelas de aplicação).
...
-- Rollback:
--   drop function if exists public.get_advisor_summary();
